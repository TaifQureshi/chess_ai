...
class Board:
    def __init__(self):
        ...
        self.board[0][0] = Rook(0, 0, "b",'r')
        self.board[0][1] = Knight(0, 1, "b",'n')
        self.board[0][2] = Bishop(0, 2, "b",'b')
        self.board[0][3] = Queen(0, 3, "b",'q')
        self.board[0][4] = King(0, 4, "b",'k')
        self.board[0][5] = Bishop(0, 5, "b",'b')
        self.board[0][6] = Knight(0, 6, "b",'n')
        self.board[0][7] = Rook(0, 7, "b",'r')

        self.board[1][0] = Pawn(1, 0, "b",'p')
        self.board[1][1] = Pawn(1, 1, "b",'p')
        self.board[1][2] = Pawn(1, 2, "b",'p')
        self.board[1][3] = Pawn(1, 3, "b",'p')
        self.board[1][4] = Pawn(1, 4, "b",'p')
        self.board[1][5] = Pawn(1, 5, "b",'p')
        self.board[1][6] = Pawn(1, 6, "b",'p')
        self.board[1][7] = Pawn(1, 7, "b",'p')

        self.board[7][0] = Rook(7, 0, "w",'r')
        self.board[7][1] = Knight(7, 1, "w",'n')
        self.board[7][2] = Bishop(7, 2, "w",'b')
        self.board[7][3] = Queen(7, 3, "w",'q')
        self.board[7][4] = King(7, 4, "w",'k')
        self.board[7][5] = Bishop(7, 5, "w",'b')
        self.board[7][6] = Knight(7, 6, "w",'n')
        self.board[7][7] = Rook(7, 7, "w",'r')

        self.board[6][0] = Pawn(6, 0, "w",'p')
        self.board[6][1] = Pawn(6, 1, "w",'p')
        self.board[6][2] = Pawn(6, 2, "w",'p')
        self.board[6][3] = Pawn(6, 3, "w",'p')
        self.board[6][4] = Pawn(6, 4, "w",'p')
        self.board[6][5] = Pawn(6, 5, "w",'p')
        self.board[6][6] = Pawn(6, 6, "w",'p')
        self.board[6][7] = Pawn(6, 7, "w",'p')
    ...
    def make_move(self,chess_coord,piece):
        ...
        if piece.rook:
            if piece.row == 0 and piece.col == 0:
                if self.board[0][4] != 0 and self.board[0][4].king:
            ...
            if piece.row == 7 and piece.col == 0:
                if self.board[7][4] != 0 and self.board[7][4].king:
        ...
        if piece.pawn:
            if piece.color == "w":
                if piece.row == 0:
                    self.board[row][col] = Queen(row, col, "w",'q')         
            else:
                if piece.row == 7:
                    self.board[row][col] = Queen(row, col, "b",'q')
            ...
            for j in range(8):
                if self.board[i][j] != 0:
                    ...
                    if self.board[i][j].color != color and not self.board[i][j].king:
                        moves = self.board[i][j].update_move(self)
                        for move in moves:
                            danger.append(move)
